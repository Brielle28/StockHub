...
const Navbar = ({ toggleSidebar }) => {
  ...
  // FIX 1: Watch for new triggered alerts directly
  useEffect(() => {
    // Listen for the most recent alert (first item in array since it's added to the beginning)
    if (triggeredAlerts.length > 0) {
      const mostRecentAlert = triggeredAlerts[0];
      
      // Show toast for the most recent alert
      // Use a unique key to ensure we don't show the same alert twice
      const alertKey = `${mostRecentAlert.alertId}-${mostRecentAlert.symbol}-${mostRecentAlert.targetPrice}`;
      
      // Store the alert key to prevent duplicate toasts
      const lastShownAlert = localStorage.getItem('lastShownAlert');
      if (lastShownAlert !== alertKey) {
        console.log('Showing toast for new alert:', mostRecentAlert);
        setNewAlertToast(mostRecentAlert);
        setHasUnreadNotifications(true);
        localStorage.setItem('lastShownAlert', alertKey);
      }
    }
  }, [triggeredAlerts]); // Watch the entire triggeredAlerts array
  ...
  }, [triggeredAlerts.length, hasUnreadNotifications]);
